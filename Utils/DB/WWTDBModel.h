//
//  WWTDBModel.h
//  WWTKit
//
//  Created by eamon on 2019/4/9.
//  Copyright © 2019年 wewave Inc. All rights reserved.
//

#import <Foundation/Foundation.h>



//------------------------------------------------------------------------
#pragma mark - Constants
//------------------------------------------------------------------------

//MARK: >> WWTDBDataType
//------------------------------------------------------------------------
typedef NSString * const WWTDBDataType;

extern WWTDBDataType WWTDBDataType_TEXT;
extern WWTDBDataType WWTDBDataType_INTEGER;
extern WWTDBDataType WWTDBDataType_REAL;
extern WWTDBDataType WWTDBDataType_BLOB;
extern WWTDBDataType WWTDBDataType_NULL;

extern NSString * const WWTDBPrimaryKey;
extern NSString * const WWTDBPrimaryId;


//------------------------------------------------------------------------
#pragma mark - Interface (WWTDBModel)
//------------------------------------------------------------------------

@interface WWTDBModel : NSObject

/// 主键 id
@property (assign, nonatomic) NSInteger primaryKey;

/// 列名
@property (strong, readonly, nonatomic) NSMutableArray *columeNames;

/// 列类型
@property (strong, readonly, nonatomic) NSMutableArray *columeTypes;


//------------------------------------------------------------------------
#pragma mark - Key
//------------------------------------------------------------------------

/**
 *  获取该类的所有属性, 不包括主键
 */
+ (NSDictionary *)getPropertys;

/**
 获取所有属性，包括主键
 */
+ (NSDictionary *)getAllProperties;

/**
 数据库中是否存在表
 */
+ (BOOL)isExistInTable;

/**
 表中的字段
 */
+ (NSArray *)getColumns;


//------------------------------------------------------------------------
#pragma mark - Insert or Update
//------------------------------------------------------------------------

/**
 保存或更新
 如果不存在主键，保存，
 有主键，则更新
 */
- (BOOL)saveOrUpdate;

/**
 保存或更新
 如果根据特定的列数据可以获取记录，则更新，
 没有记录，则保存
 */
- (BOOL)saveOrUpdateByColumnName:(NSString*)columnName AndColumnValue:(NSString*)columnValue;


/**
 保存单个数据
 */
- (BOOL)save;

/**
 批量保存数据
 事务方式插入数据库
 */
+ (BOOL)saveObjects:(NSArray *)array;

/**
 更新单个数据
 */
- (BOOL)update;

/**
 批量更新数据
 */
+ (BOOL)updateObjects:(NSArray *)array;


//------------------------------------------------------------------------
#pragma mark - Delete
//------------------------------------------------------------------------

/**
 删除单个数据
 */
- (BOOL)deleteObject;

/**
 批量删除数据
 */
+ (BOOL)deleteObjects:(NSArray *)array;

/**
 通过条件删除数据
 */
+ (BOOL)deleteObjectsByCriteria:(NSString *)criteria;

/**
 通过条件删除 (多参数）
 */
+ (BOOL)deleteObjectsWithFormat:(NSString *)format, ...;


//------------------------------------------------------------------------
#pragma mark - Find
//------------------------------------------------------------------------

/**
 查询全部数据
 */
+ (NSArray *)findAll;

/**
 通过主键查询
 */
+ (instancetype)findByPK:(int)inPk;

/**
 查找单条数据
 */
+ (instancetype)findFirstByCriteria:(NSString *)criteria;

/**
 条件查找多条数据
 这样可以进行分页查询 @" WHERE pk > 5 limit 10"
 */
+ (NSArray *)findByCriteria:(NSString *)criteria;

//------------------------------------------------------------------------
#pragma mark - Table
//------------------------------------------------------------------------

/**
 清空表
 */
+ (BOOL)clearTable;

/**
 创建表
 如果已经创建，返回YES
 */
+ (BOOL)createTable;


//------------------------------------------------------------------------
#pragma mark - must be override method
//------------------------------------------------------------------------

/**
 如果子类中有一些property不需要创建数据库字段，那么这个方法必须在子类中重写
 */
+ (NSArray *)transients;


@end
